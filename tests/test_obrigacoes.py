...
def com_darf(conn):
    """Venda que gera DARF de R$ 1.500 na competência 06/2026, vencendo em 31/07."""
    conn.executemany(
        ...
        [("2026-01-05", "COMPRA", 1000, 20, 20_000),
         ("2026-06-10", "VENDA", 1000, 30, 30_000)])
    return conn
...
def test_vencido_calcula_multa_de_mora(conn):
    ...
    assert o.juros is None                                      # exige a Selic
    assert o.total_a_pagar == pytest.approx(1_500 + o.multa)
...
def test_juros_so_com_a_selic():
    _, _, sem = obrigacoes.encargos(1_000, "2026-07-31", "2026-08-10")
    assert sem is None
    _, _, com = obrigacoes.encargos(1_000, "2026-07-31", "2026-08-10", 0.0102)
    assert com == pytest.approx(10.20)
